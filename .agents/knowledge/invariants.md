...
- **Fix prompt/response problems through the AI, never by hardcoding**: When the AI produces the wrong output for a given input, the fix must go through one of these layers: (1) the system prompt, (2) tool declarations, (3) structured output schema, or (4) adding entity-type context to the message (e.g., "the X dataset" not just "X"). Never fix it by replacing a natural-language message with a hardcoded command (e.g., "List the tables in X"), adding keyword detection, or creating if/else branches based on the user's words. Click and chip messages are prompts -- they must be natural language that the AI interprets, not commands that bypass interpretation.
...
- **Preserve raw model parts in function-call loops**: When appending the model's function-call turn to the `contents` array, always use the raw `candidate.content.parts` from the API response. Never synthetically reconstruct parts from just `{ name, args }`. Raw parts contain opaque fields like `thoughtSignature` that the API requires on subsequent turns. Both `loop.ts` and the adapter path in `gemini-client.ts` use `rawModelParts` from the adapter response for this.
...
- **ABSOLUTE RULE -- Never guess at bug causes. Always observe actual behavior first.** When debugging any issue: (1) capture what tools the agent actually called and with what args, (2) capture what envelope type was built and why, (3) capture what the LLM's text response was. Use the execution trace, console logs, or browser DevTools. Do NOT theorize about what "might" be happening. Do NOT make code changes based on theories. Look at the actual data first, then fix the actual cause.
...
- **Sidebar group for saved items is labeled "Library"** (renamed from "Content"): The SideNav group header, page headers, save confirmation messages, and button tooltips all use "Library". Do not use "Content", "Saved Work", or "Saved" as the user-facing label for this section.
- **Pin = chats, Favorite = Library items**: Pins (`bqaif_pinned_chats` in localStorage) are only for chat sessions. Favorites (the `pinned: boolean` field on `SavedArtifact`) are for Library items. The FavoritesPage shows both. Do not conflate these concepts.
- **TabBar hides on sidebar overlay pages when only the chat tab exists**: When `activePage` is a non-chat overlay (favorites, spaces, prompts, templates) AND `activeTabId === 'chat'`, TabBar returns null. It stays visible when a non-chat tab exists so the user can navigate back.
- **`closeTab()` resets `activePage` to `'chat'`**: The `closeTab` callback in `page-context.tsx` must set `activePage` to `'chat'` after removing the tab. Skipping this caused stale overlay states after closing the last non-chat tab.
- **Open non-chat tabs appear in the sidebar**: SideNav shows an "Open" group below the Library group listing all non-chat tabs. This gives users a way to return to builder/dashboard tabs after navigating to a sidebar page.
- **Builder documents on the All tab**: SavedPage shows builder documents on both the "All" and "Documents" tabs. Documents are fetched from both in-memory (`builder.getOpenDocuments()`) and Firestore (`getBuilderDocuments(userId)`), merged and deduplicated by ID.
