...
- **Query auto-retry is implicit**: Errors from `run_query` tool calls feed back to the LLM as function responses. The LLM can fix its SQL and call `run_query` again within the 6-iteration cap. Do not add explicit retry logic.
- **Tool-calling loop cap is 6 iterations**: `callGeminiWithTools()` stops after 6 rounds to prevent runaway loops. Do not increase without measuring latency impact.
