...
- **Tool-calling loop cap is 15 iterations for query, 8 for conversation**: `callGeminiWithTools()` defaults to 8 iterations. `handleQuery()` overrides to 15 to support complex widget requests. Do not increase the query cap above 15 without measuring latency. Do not increase the conversation cap above 8.
