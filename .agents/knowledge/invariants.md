...
- **Cloud Function invoker must be public**: The `geminiProxy` Cloud Function v2 must use `invoker: "public"`. Firebase Hosting rewrites cannot pass IAM credentials, so `invoker: "private"` causes Google Cloud to return an HTML 403 before the function code runs. Application-level auth (Firebase ID token verification in the function) is the correct security layer.
...
- **NEVER use keywords to route data tasks — the LLM understands intent in any phrasing.** Keyword routing (adding words to MUTATING_VERBS, QUERY_SIGNALS, or any manifest signal list) to fix a routing problem is always wrong for data task dispatch. Keywords require exhausting every synonym a user might say ("delete", "remove", "get rid of", "clean out", "wipe", "purge"...) and will always have gaps. The correct pattern for data operations: let the conversation agent's LLM write the SQL, then intercept at the **execution layer** to add safety gates (preview count, confirm card). This way any phrasing works, not just the ones we anticipated. If a data operation is not being handled correctly, fix the system prompt or the execution-layer intercept — never add keywords.
...
- **Destructive DML is intercepted in the conversation handler**: When the conversation agent's `execute_dml` tool receives a DELETE, TRUNCATE, or DROP statement, it does NOT execute immediately. Instead it sets `pendingDestructiveDml`, returns a `pending_confirmation` signal to the LLM, and after the agent loop builds a CONFIRMATION_CARD envelope using the LLM's generated SQL. This ensures any phrasing (no keywords needed) routes destructive operations through the preview+confirm flow. Do NOT remove this intercept or call `executeDml` directly for these statement types.
