...
| "show me how many total sales for store X" | query | Single aggregation with filter -- one verb, one object. NEVER multistep. |
| "how many orders for customer Y" | query | Filtered count -- single-step query producing a KPI card |
| "total revenue by category" | query | Aggregation with grouping -- single-step query |

### Aggregation Anti-Pattern

**Aggregation queries are ALWAYS single-step.** Queries like "how many X for Y", "total X for Y", "average X by Z" require ONE step: the query skill. Do NOT decompose these into schema+query workflows. The query skill loads schema context internally -- there is never a reason to pre-fetch schema as a separate step.
