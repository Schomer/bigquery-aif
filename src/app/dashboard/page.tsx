...
// shows last cached snapshot instantly, then replaces with fresh data.
...
function TileResult({ columns, rows }: { columns: string[]; rows: (string | number | boolean | null)[][] }) {
  const MAX_ROWS = 6;
  ...
  if (columns.length === 1 && rows.length === 1) {
    return (
      <div style={{ fontSize: 30, fontWeight: 700, color: 'var(--text)', letterSpacing: '-0.5px', padding: '8px 0' }}>
        {String(rows[0][0] ?? '—')}
      </div>
    );
  }
  ...
  const isBar = columns.length === 2 && displayRows.length > 0 && displayRows.every(r => !isNaN(Number(r[1])));
  if (isBar) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5, padding: '4px 0' }}>
        {displayRows.map((row, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontSize: 10, color: 'var(--text-muted)', width: 90, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flexShrink: 0 }}>{String(row[0] ?? '')}</span>
            <div style={{ flex: 1, height: 12, background: 'var(--surface-2, #f3f4f6)', borderRadius: 2, overflow: 'hidden' }}>
              <div style={{ width: `${(values[i] / maxVal) * 100}%`, height: '100%', background: '#1967d2', borderRadius: 2 }} />
            </div>
    ...
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', fontSize: 11, borderCollapse: 'collapse' }}>
        <thead>
          <tr>
            {columns.map(c => (
              <th key={c} style={{ textAlign: 'left', padding: '2px 6px 4px', color: 'var(--text-dim)', fontWeight: 600, borderBottom: '1px solid var(--border)', whiteSpace: 'nowrap', fontSize: 10 }}>{c}</th>
            ))}
          </tr>
        </thead>
        <tbody>
            ...
            <tr key={i}>
              {row.map((cell, j) => (
                <td key={j} style={{ padding: '3px 6px', color: 'var(--text)', whiteSpace: 'nowrap', maxWidth: 130, overflow: 'hidden', textOverflow: 'ellipsis', borderBottom: '1px solid var(--border-subtle, #f0f0f0)' }}>
                  {cell === null || cell === undefined ? <span style={{ color: 'var(--text-dim)', fontStyle: 'italic' }}>null</span> : String(cell)}
                </td>
              ))}
            </tr>
          ))}
...
function ArtifactPicker({ artifacts, onAdd, onClose }: { artifacts: SavedArtifact[]; onAdd: (a: SavedArtifact) => void; onClose: () => void }) {
  ...
  return (
    ...
    >
      <div
        style={{ background: 'var(--surface)', borderRadius: 14, width: 440, maxHeight: '72vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', overflow: 'hidden', fontFamily: "'Google Sans', sans-serif" }}
      ...
      >
        <div style={{ padding: '16px 18px 10px', borderBottom: '1px solid var(--border)' }}>
          ...
          <input
            ...
            style={{ width: '100%', padding: '8px 12px', border: '1px solid var(--border)', borderRadius: 8, fontSize: 13, outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }}
          />
        </div>
        <div style={{ flex: 1, overflow: 'auto', padding: '8px 10px' }}>
          ...
          {filtered.map(a => (
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '10px 12px', border: 'none', background: 'none', cursor: 'pointer', borderRadius: 8, textAlign: 'left', fontFamily: 'inherit' }}
...
export default function DashboardPage() {
  ...
  useEffect(() => { if (uid) load(uid); }, [uid, load]);
  ...
  const runTileQueries = useCallback(async (dashboard: SavedDashboard, proj: string) => {
    ...
    const tilesWithSql = dashboard.tiles.filter(t => t.cachedSql);
  ...
  async function addTile(artifact: SavedArtifact) {
    ...
    const tile: DashboardTile = { id: generateId(), artifactId: artifact.id, title: artifact.name, col: 0, row: nextRow, colSpan: 6, rowSpan: 2, cachedSql: sql || undefined };
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0 20px', height: 52, borderBottom: '1px solid var(--border)', background: 'var(--surface)', flexShrink: 0, position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ position: 'relative' }}>
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', border: '1px solid var(--border)', borderRadius: 8, background: 'var(--surface)', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 600, color: 'var(--text)' }}
          >
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 20 }}>
        {!activeDashboard ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '80%', gap: 12, color: 'var(--text-muted)', border: '2px dashed var(--border)', borderRadius: 14 }}>
            ...
            <div style={{ fontSize: 14 }}>Click "Add tile" to add your first saved query result.</div>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(${COLS}, 1fr)`, gap: 12, alignItems: 'start' }}>
            {activeDashboard.tiles.map(tile => {
              ...
              return (
                <div key={tile.id} style={{ gridColumn: `span ${tile.colSpan}`, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 6, minHeight: tile.rowSpan * 100 }}>
                  {/* Header */}
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ flex: 1, fontSize: 12, fontWeight: 600, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{tile.title}</span>
                    {status === 'loading' && <span style={{ fontSize: 10, color: '#1967d2' }}>Refreshing...</span>}
                    {snap && status !== 'loading' && <span style={{ fontSize: 9, color: 'var(--text-dim)' }}>{new Date(snap.fetchedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>}
                    <button onClick={() => removeTile(tile.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-dim)', padding: 2, flexShrink: 0 }} title="Remove">
                  ...
                  {/* Content */}
                  <div style={{ flex: 1 }}>
                    {status === 'error' && <div style={{ fontSize: 11, color: '#c62828' }}>Query failed</div>}
                    {!snap && status !== 'error' && <div style={{ fontSize: 11, color: 'var(--text-dim)' }}>{tile.cachedSql ? 'Click Refresh to load' : 'No SQL attached'}</div>}
                    {snap && <TileResult columns={snap.columns} rows={snap.rows} />}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 3, paddingTop: 4, alignItems: 'center', flexWrap: 'wrap', borderTop: '1px solid var(--border-subtle, #f0f0f0)', marginTop: 4 }}>
                    ...
                    {[3,4,6,8,12].map(n => <button key={n} onClick={() => updateTileSpan(tile.id, n, tile.rowSpan)} style={{ fontSize: 9, padding: '1px 4px', borderRadius: 3, background: tile.colSpan === n ? '#1967d2' : 'var(--border)', color: tile.colSpan === n ? 'white' : 'var(--text-muted)', border: 'none', cursor: 'pointer' }}>{n}</button>)}
                    <span style={{ fontSize: 9, color: 'var(--text-dim)', marginLeft: 4 }}>H:</span>
                    {[1,2,3,4].map(n => <button key={n} onClick={() => updateTileSpan(tile.id, tile.colSpan, n)} style={{ fontSize: 9, padding: '1px 4px', borderRadius: 3, background: tile.rowSpan === n ? '#1967d2' : 'var(--border)', color: tile.rowSpan === n ? 'white' : 'var(--text-muted)', border: 'none', cursor: 'pointer' }}>{n}</button>)}
                  </div>
                </div>
              );
            })}
