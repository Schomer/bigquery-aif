...
// Lightweight endpoint for table preview: sample rows + column profile.
// Called directly by SchemaView — no AI involvement.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { tableRef, columns, project } = body;

    if (!tableRef) {
      return NextResponse.json({ error: 'tableRef is required' }, { status: 400 });
    }

    // ── Run sample + profile in parallel ────────────────────────────────────
